...
| Server | Node.js (lightweight HTTP + WebSocket) |
| Real-time communication | WebSocket (ws library) |
| Audio | Web Audio API |
| Hosting | No build step required; serve static HTML |
...
| Session ID | Displayed top-right; auto-assigned unique code |
| Lobby display | Player names shown in local state; LAN integration planned for v1.2 (shared player list) |
...
|---|---|---|---|
| van | Equipment Van | No | Contains tool shelf; ghost cannot enter; safe zone during hunts |
...
**Ghost room selection:** One ghost-eligible room is randomly selected per session (house only, not van). All evidence events only fire when players are inside that room. The ghost room ID is **never sent to the client** — players must infer it from evidence.
...
**Exit door:** On the south wall of the Foyer. Locked for all players during active hunts. Door to van in foyer.

**Van tool shelf:** Physical tool pickup system. Players enter the van at game start. 8 tools are displayed on a shelf. Press `E` to pick up; press `G` to drop. Limited to 2-slot inventory.
...
| Door lock | Van door is locked during hunts, preventing entry/exit; blocks crossing at foyer boundary |
...
| Deployment | Static HTML file + Node.js server; no build step required |
...
## 17. Known Limitations (v1.1 Beta)

- **No player avatars:** Other players' positions and state are synced server-side but not rendered in the 3D world.
- **No shared lobby display:** Player names and counts are not yet broadcast to other players' lobbies.
...
- Add shared player list in lobby with live join notifications
